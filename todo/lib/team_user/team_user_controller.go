...
func (h controller) getCommonTeams(c *fiber.Ctx) error {
	...
	if err := h.teamService.db.Joins("teams inner join (team_users T1 inner join team_users T2 on T2.user_id = ?) on T1.user_id = ? and T1.team_id = T2.team_id", friendID, userID).Find(&commonTeams).Error; err != nil {
